...
impl TryFrom<&proto::GenesisId> for GenesisId {
    ...
    fn try_from(p :&proto::GenesisId) -> anyhow::Result<Self> {
        Ok(Self{
            ...
            hash: (||CryptoHash::try_from(p.hash.as_ref().context("missing")?))().context("hash")?,
        })
    }
}
...
impl TryFrom<&proto::PeerChainInfo> for PeerChainInfoV2 {
    ...
    fn try_from(p :&proto::PeerChainInfo) -> anyhow::Result<Self> {
        Ok(Self{
            genesis_id: (||GenesisId::try_from(p.genesis_id.as_ref().context("missing")?))().context("genesis_id")?,
            height: p.height,
...
impl TryFrom<&Handshake> for proto::Handshake {
    ...
    fn try_from(x:&Handshake) -> anyhow::Result<Self> {
        Ok(Self{
            ...
            sender_chain_info: Some((&x.sender_chain_info).try_into().context("sender_chain_info")?),
            partial_edge_info: Some((&x.partial_edge_info).try_into().context("partial_edge_info")?),
        })
    }
}
...
impl TryFrom<(&PeerInfo,&HandshakeFailureReason)> for proto::HandshakeFailure {
    ...
    fn try_from((pi,hfr):(&PeerInfo,&HandshakeFailureReason)) -> anyhow::Result<Self> {
        Ok(match hfr {
            HandshakeFailureReason::ProtocolVersionMismatch{version,oldest_supported_version} => Self{
...
impl TryFrom<&proto::HandshakeFailure> for (PeerInfo,HandshakeFailureReason) {
    ...
    fn try_from(x:&proto::HandshakeFailure) -> anyhow::Result<Self> {
        let pi = (||PeerInfo::try_from(x.peer_info.as_ref().context("missing")?))().context("peer_info")?;
        let hfr = match proto::handshake_failure::Reason::from_i32(x.reason).context("unknown reason")? {
            proto::handshake_failure::Reason::ProtocolVersionMismatch => HandshakeFailureReason::ProtocolVersionMismatch{
                version: x.version,
                oldest_supported_version: x.oldest_supported_version,
            },
            proto::handshake_failure::Reason::GenesisMismatch => HandshakeFailureReason::GenesisMismatch(
                (||GenesisId::try_from(x.genesis_id.as_ref().context("missing")?))().context("genesis_id")?
            ),
            proto::handshake_failure::Reason::InvalidTarget => HandshakeFailureReason::InvalidTarget,
...
impl TryFrom<&RoutingTableUpdate> for proto::RoutingTableUpdate {
    ...
    fn try_from(x:&RoutingTableUpdate) -> anyhow::Result<Self> {
        ...
        for e in &x.edges { p.edges.push(e.try_into().context("edges")?); }
        for a in &x.accounts { p.accounts.push(a.try_into().context("accounts")?); }
        Ok(p)
    }
}
...
impl TryFrom<&PeerMessage> for proto::PeerMessage {
    ...
    fn try_from(x:&PeerMessage) -> anyhow::Result<Self> {
        Ok(Self{message_type: Some(match x {
            PeerMessage::Handshake(h) => ProtoMT::Handshake(h.try_into().context("Handshake")?),
            PeerMessage::HandshakeFailure(pi,hfr) => ProtoMT::HandshakeFailure((pi,hfr).try_into().context("HandshakeFailure")?),
            PeerMessage::LastEdge(e) => ProtoMT::LastEdge(proto::LastEdge{edge:Some(e.try_into().context("LastEdge")?)}),
            PeerMessage::SyncRoutingTable(rtu) => ProtoMT::SyncRoutingTable(rtu.try_into().context("SyncRoutingTable")?),
            PeerMessage::RequestUpdateNonce(pei) => ProtoMT::UpdateNonceRequest(proto::UpdateNonceRequest{
                partial_edge_info:Some(pei.try_into().context("UpdateNonceRequest")?),
            }),
            PeerMessage::ResponseUpdateNonce(e) => ProtoMT::UpdateNonceResponse(proto::UpdateNonceResponse{
            ...
            PeerMessage::PeersResponse(pis) => ProtoMT::PeersResponse(proto::PeersResponse{
                peers: {
                    let mut peers = vec![];
                    for pi in pis { peers.push(pi.try_into().context("PeersResponse")?); }
                    peers
                },
            }),
            PeerMessage::BlockHeadersRequest(bhs) => ProtoMT::BlockHeadersRequest(proto::BlockHeadersRequest{
                block_hashes: {
                    let mut block_hashes = vec![];
                    for bh in bhs { block_hashes.push(bh.try_into().context("BlockHeadersRequest")?); }
                    block_hashes
                },
            }),
            PeerMessage::BlockHeaders(bhs) => ProtoMT::BlockHeadersResponse(proto::BlockHeadersResponse{
                block_headers: {
                    let mut block_headers = vec![];
                    for bh in bhs { block_headers.push(bh.try_into().context("BlockHeadersResponse")?); }
                    block_headers
                },
            }),
            ...
            PeerMessage::EpochSyncResponse(esr) => ProtoMT::EpochSyncResponse(proto::EpochSyncResponse{
                borsh: esr.try_to_vec().context("EpochSyncResponse")?,
            }),
            PeerMessage::EpochSyncFinalizationRequest(epoch_id) => ProtoMT::EpochSyncFinalizationRequest(proto::EpochSyncFinalizationRequest{
                epoch_id: Some((&epoch_id.0).try_into().context("EpochSyncFinalizationRequest")?),
            }),
            PeerMessage::EpochSyncFinalizationResponse(esfr) => ProtoMT::EpochSyncFinalizationResponse(proto::EpochSyncFinalizationResponse{
                borsh: esfr.try_to_vec().context("EpochSyncFinalizationResponse")?,
            }),
            PeerMessage::RoutingTableSyncV2(rs) => ProtoMT::RoutingTableSyncV2(proto::RoutingSyncV2{
...
impl TryFrom<&proto::PeerMessage> for PeerMessage {
    ...
    fn try_from(x:&proto::PeerMessage) -> anyhow::Result<Self> {
        Ok(match x.message_type.as_ref().context("empty or unknown")? {
            ...
            ProtoMT::SyncRoutingTable(rtu) => PeerMessage::SyncRoutingTable(rtu.try_into().context("SyncRoutingTable")?),
            ProtoMT::UpdateNonceRequest(unr) => PeerMessage::RequestUpdateNonce(
                (||PartialEdgeInfo::try_from(unr.partial_edge_info.as_ref().context("missing")?))().context("UpdateNonceRequest")?
            ),
            ProtoMT::UpdateNonceResponse(unr) => PeerMessage::ResponseUpdateNonce(
                (||Edge::try_from(unr.edge.as_ref().context("missing")?))().context("UpdateNonceResponse")?
            ),
            ProtoMT::PeersRequest(_) => PeerMessage::PeersRequest,
            ProtoMT::PeersResponse(pr) => PeerMessage::PeersResponse({
                let mut pis = vec![];
                for pi in &pr.peers { pis.push(pi.try_into().context("PeersResponse")?); }
                pis
            }),
            ProtoMT::BlockHeadersRequest(bhr) => PeerMessage::BlockHeadersRequest({
                let mut bhs = vec![];
                for bh in &bhr.block_hashes { bhs.push(bh.try_into().context("BlockHeadersRequest")?); }
                bhs
            }),
            ProtoMT::BlockHeadersResponse(bhr) => PeerMessage::BlockHeaders({
                let mut bhs = vec![];
                for bh in &bhr.block_headers { bhs.push(bh.try_into().context("BlockHeadersResponse")?); }
                bhs
            }),
            ProtoMT::BlockRequest(br) => PeerMessage::BlockRequest(
                (||CryptoHash::try_from(br.block_hash.as_ref().context("missing")?))().context("BlockRequest")?
            ),
            ProtoMT::BlockResponse(br) => PeerMessage::Block(
                (||Block::try_from(br.block.as_ref().context("missing")?))().context("BlockResponse")?
            ),
            ProtoMT::Transaction(t) => PeerMessage::Transaction(
                SignedTransaction::try_from_slice(&t.borsh).context("Transaction")?,
            ),
            ProtoMT::Routed(r) => PeerMessage::Routed(
                Box::new(RoutedMessage::try_from_slice(&r.borsh).context("Routed")?)
            ),
            ProtoMT::Disconnect(_) => PeerMessage::Disconnect,
            ProtoMT::Challenge(c) => PeerMessage::Challenge(
                Challenge::try_from_slice(&c.borsh).context("Challenge")?
            ),
            ProtoMT::EpochSyncRequest(esr) => PeerMessage::EpochSyncRequest(EpochId(
                (||CryptoHash::try_from(esr.epoch_id.as_ref().context("missing")?))().context("EpochSyncRequest")?
            )),
            ProtoMT::EpochSyncResponse(esr) => PeerMessage::EpochSyncResponse(
                Box::new(EpochSyncResponse::try_from_slice(&esr.borsh).context("EpochSyncResponse")?)
            ),
            ProtoMT::EpochSyncFinalizationRequest(esr) => PeerMessage::EpochSyncFinalizationRequest(EpochId(
                (||CryptoHash::try_from(esr.epoch_id.as_ref().context("missing")?))().context("EpochSyncFinalizationRequest")?
            )),
            ProtoMT::EpochSyncFinalizationResponse(esr) => PeerMessage::EpochSyncFinalizationResponse(
                Box::new(EpochSyncFinalizationResponse::try_from_slice(&esr.borsh).context("EpochSyncFinalizationResponse")?)
            ),
