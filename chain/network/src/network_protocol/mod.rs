...
impl PeerMessage {
    pub(crate) fn serialize(&self, mode:Mode) -> anyhow::Result<Vec<u8>> {
        Ok(match mode {
            Mode::Borsh => borsh::PeerMessage::try_from(self)?.try_to_vec()?,
            Mode::Proto => proto::PeerMessage::try_from(self)?.encode_to_vec(),
        })
    }

    pub(crate) fn deserialize(mode:Mode, data:&[u8]) -> anyhow::Result<PeerMessage> {
        Ok(match mode {
            Mode::Borsh => (&borsh::PeerMessage::try_from_slice(data)?).try_into()?,
            Mode::Proto => (&proto::PeerMessage::decode(data)?).try_into()?,
        })
    }
