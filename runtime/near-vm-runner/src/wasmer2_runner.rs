...
fn get_entrypoint_index(
    engine: &dyn wasmer_engine::Engine,
    artifact: &VMArtifact,
    method_name: &str,
    ...
    if let Some(index) = artifact.function_by_export_field(method_name) {
        let signature = artifact.function_signature(index).expect("index should produce signature");
        let signature = engine.lookup_signature(signature).expect("signature store invlidated?");
...
impl Wasmer2VM {
    ...
    fn run_method(
        ...
        let entrypoint = get_entrypoint_index(artifact.engine(), &*artifact, method_name)?;
        unsafe {
            let instance = {
    ...
    pub(crate) fn run_module<'a>(
        ...
        if let Err(e) = get_entrypoint_index(artifact.engine(), &artifact, method_name) {
            return (None, Some(e));
        }
